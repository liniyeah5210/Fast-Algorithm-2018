...
/*
C24031180
The DCT here is the same as the DCT in matlab.
The DST here is the DST-II on wiki.
*/
int FFT(double *x_r, double *x_i, double *y_r, double *y_i, int N);

int main()
{
	int i, N, flag, K,print, *p;
	...
	scanf("%d", &N);
	printf("N=%d\n", N);
	printf("1: DCT(same as DCT in matlab), 2: DST(DST-II on wiki) \n");
	printf("Enter 1 or 2:  ");
	scanf("%d", &flag);
	printf("print=");
	scanf("%d", &print);
	...
	//DCT: scaling 
	y_r[0] = sqrt(1./N)*y_r[0];
	for(i=1;i<N;++i)
	{
		y_r[i] = sqrt(2./N)*y_r[i];
	}


	//DST: let it become positive and shift the index 
	for(i=0;i<N;++i)
	{
		y_i[i] = -y_i[i+1];
	}
