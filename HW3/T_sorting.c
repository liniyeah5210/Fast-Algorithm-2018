...
int main()
{
	...
	srand( time(NULL) );
		...
		#if DEBUG2        // if DEBUG == 1, then compile the following codes 
		for(i=0;i<N;++i)
		{
			printf("x[%d]=%d\n",i,x[i]);
		}
...
int quicksort1(int *x, int left, int right)
{
	...
	if(left < right-1)
	{
		// ¥ý§ä¤@­Ó pivot, µM«á§â¤ñ pivot ¤pªº ©ñ¨ì«e­±, ¤ñpivot ¤jªº©ñ¨ì«á­±
		// 	  ·Q¿ìªk°µ 
		// ¦A¥h±Æ pivot ªº¥ª¥k¨âÃä 
...
int quicksort2(int *x, int left, int right)
{
	...
	if(left < right-1)
	{
    	...
    	// 
    	while(1)
		{	// x: 5(pivot) 4 8 3 2 7 3 2 10  -> i = 2, j = 7 (¥æ´« x[i], x[j]) 
			//    5(pivot) 4 2 3 2 7 3 8 10  -> i = 5, j = 6 (¥æ´« x[i], x[j])
			//    5(pivot) 4 2 3 2 3 7 8 10  -> i = 6, j = 5 (¤£¥æ´«¤F!!!) 
			//    3                5 (location: j) 
			// x: 4(pivot) 4 8 3 2 7 3 4 10  -> i = 2, j = 7 (¥æ´« x[i], x[j]) 
			//    4(pivot) 4 4 3 2 7 3 8 10  -> i = 5, j = 6 (¥æ´« x[i], x[j])
			//    4(pivot) 4 4 3 2 3 7 8 10  -> i = 6, j = 5 (¤£¥æ´«¤F!!!) 
			//    3                4 (location: j) 			
			// x: 8 2 1 8 7 8 9 4 5 8      i = 3, j = 9 (¥æ´« x[i], x[j])
			// x: 8 2 1 8 7 8 9 4 5 8
      		while(i < right && pivot >= x[i]) i++; // ©¹¥kÃä§ä¨ì²Ä¤@­Ó  pivot <  x[i]  
      		while(j >  left && pivot <  x[j]) j--; // ©¹¥ªÃä§ä¨ì²Ä¤@­Ó  pivot >= x[j] 
      		#if DEBUG2
			printf("%d %d %d\n", i,j,pivot);
			#endif
      		if(i>=j) break;
