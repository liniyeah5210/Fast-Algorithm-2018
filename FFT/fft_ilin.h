...
//C24031180 I-Lin Yeh
//If you want to measure the time, please use the function "omp_get_wtime()" from omp.h library. Thanks!!
	...
	*/
	...
	} else 
	{
		...
		} else if(N % 5 == 0)
		{
			...
			a_r = cos(1*2*M_PI/5); b_r = cos(2*2*M_PI/5); c_r=cos(3*2*M_PI/5); d_r=cos(4*2*M_PI/5);
			a_i = -sin(1*2*M_PI/5); b_i = -sin(2*2*M_PI/5); c_i=-sin(3*2*M_PI/5); d_i=-sin(4*2*M_PI/5);
			#pragma omp parallel for private (k, w_re, w_im, w2_re, w2_im, w3_re, w3_im, w4_re, w4_im, u1_r, u1_i, u2_r, u2_i, u3_r, u3_i, u4_r, u4_i)
			for(k=0;k<N/5;++k)
			{
				w_re = cos(k*2*M_PI/N);
				w_im = -sin(k*2*M_PI/N);
